...
// ─── SVG Previews ─────────────────────────────────────────────────────────────
// All: viewBox="0 0 100 80", white strokes, transparent bg (green applied by wrapper)
...
function PreviewAcabados() {
  return (
    <svg viewBox="0 0 100 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Hatch pattern on tile [0,0] to indicate different material */}
...
const PLANOS: PlanoData[] = [
  {
    ...
    filename: "plano-arquitectonico-general.pdf",
    size: "4.2 MB",
  ...
  return (
    <main className="flex-1 overflow-y-auto p-6">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="text-xs text-gray-400">{PLANOS.length} planos</span>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {PLANOS.map((plano) => (
          <PlanoCard key={plano.id} plano={plano} ubicacion={user.ubicacion} />
        ))}
      </div>
    </main>
  );
