...
/**
 * Motivo por el que la unidad no puede enviar reclamos, resuelto por el
 * Server Component de page.tsx (fechaVencimiento llega ya formateada es-GT).
 * null = formulario habilitado.
 */
export type BloqueoReclamos =
  | { motivo: "vencido"; fechaVencimiento: string | null }
  | { motivo: "no_entregado" };
...
export default function ReclamosClient({ bloqueo }: { bloqueo: BloqueoReclamos | null }) {
  // Form fields
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        // 403 de elegibilidad: puede darse si la garantía venció con la página
        // ya abierta (el bloqueo del server component no llegó a renderizarse)
        const msg = json.error === "reclamo_no_permitido"
          ? "Tu unidad ya no es elegible para enviar reclamos (garantía vencida o unidad no entregada). Recarga la página para más detalle."
          : (json.error as string) ?? "Error al enviar el reclamo";
        setFormErrors({ titulo: msg });
        setSubmitting(false);
        return;
      }
    } catch {
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {bloqueo
                  ? "El envío de reclamos no está disponible para tu unidad"
                  : "Completa el formulario y adjunta evidencia fotográfica de ser necesario"}
              </p>
            </div>

            {bloqueo ? (
              <BloqueoNotice bloqueo={bloqueo} />
            ) : (
...
// Aviso mostrado en lugar del formulario cuando la unidad no es elegible
// (garantía vencida o unidad aún no entregada). Paleta de marca, no estilos
// de error: es una condición esperada del negocio, no un fallo.
function BloqueoNotice({ bloqueo }: { bloqueo: BloqueoReclamos }) {
  const Icon = bloqueo.motivo === "vencido" ? IconCalendarX : IconClock;
  return (
    <div className="p-6">
      <div className="flex items-start gap-4 p-5 rounded-2xl border border-[#e8e2d9] bg-[#f7f4f0]">
        <div className="w-10 h-10 rounded-full bg-[#2D5A3D]/10 flex items-center justify-center flex-shrink-0">
          <Icon size={20} stroke={1.75} className="text-[#2D5A3D]" />
        </div>
        <div className="min-w-0">
          {bloqueo.motivo === "vencido" ? (
            <>
              <p className="text-sm font-semibold text-gray-900">Período de garantía finalizado</p>
              <p className="text-sm text-gray-600 mt-1 leading-relaxed">
                {bloqueo.fechaVencimiento ? (
                  <>
                    El período de garantía venció el{" "}
                    <span className="font-medium text-gray-800">{bloqueo.fechaVencimiento}</span>.
                    Ya no es posible enviar reclamos para esta unidad.
                  </>
                ) : (
                  "El período de garantía de tu unidad ha finalizado. Ya no es posible enviar reclamos para esta unidad."
                )}
              </p>
            </>
          ) : (
            <>
              <p className="text-sm font-semibold text-gray-900">Unidad pendiente de entrega</p>
              <p className="text-sm text-gray-600 mt-1 leading-relaxed">
                Tu unidad aún no ha sido entregada. Podrás enviar reclamos a partir
